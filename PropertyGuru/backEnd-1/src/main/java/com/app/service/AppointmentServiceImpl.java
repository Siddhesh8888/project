...
public class AppointmentServiceImpl implements AppointmentServiceInterface {
	...
	public List<Appointment> showAllAppointmentsOfOwner(Long ownerid) {
		
		
				List<Appointment> appointments=new ArrayList<>();
				List<Appointment> all = appRepo.findAll();
				all.forEach(app->{if(app.getProperty().getOwner().getId()==ownerid) appointments.add(app);});
				return appointments;
	}
